...
function App() {
  ...
  // Capacitor Status Bar ayarları
  useEffect(() => {
    const initStatusBar = async () => {
      try {
        await StatusBar.setStyle({ style: tema === 'dark' ? Style.Dark : Style.Light })
        await StatusBar.setBackgroundColor({ color: tema === 'dark' ? '#1f2937' : '#1a2f23' })
      } catch (error) {
        // Web'de çalışmıyor, hata yok sayılabilir
        console.log('StatusBar sadece native platformlarda çalışır')
      }
    }
    initStatusBar()
  }, [tema])

  // Android Hardware Back Button desteği
  useEffect(() => {
    const backButtonListener = CapApp.addListener('backButton', ({ canGoBack }) => {
      ...
      // Eğer arama modalı açıksa kapat
      if (aramaAcik) {
        setAramaAcik(false)
        return
      }
      ...
      // Ekran geçişleri
      if (ekran === 'event-detail') {
        // Detay ekranındaysa listeye dön
        setEkran('event-list')
        setSeciliOlay(null)
      } else if (ekran === 'event-list') {
        // Liste ekranındaysa tarih seçimine dön
        setEkran('date-selection')
        setSeciliOlay(null)
      } else if (ekran === 'date-selection') {
      ...
      <header className={`shadow-lg sticky top-0 z-40 transition-colors duration-300 ${
        ...
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 flex-1 justify-center">
            ...
            >
              <img
                src="https://yt3.googleusercontent.com/06KV70YTXoPlR1zIEKHm85jVuFoRK7-tFBARwwlpTPqyB_mjI-5H5ty1OqjnOJUZ_6_22Hj-5Q=s120-c-k-c0x00ffffff-no-rj"
                alt="Logo"
